...
plot49 <- region1%>% filter(age <50) %>% group_by(region) %>% summarise(divorce_rate = mean(binary_marstat)*100)%>% ggplot(aes(x = region, y = divorce_rate))+ geom_col() + theme(axis.text.x = element_text(angle = 30, hjust = 1)) + labs( title = "Divorce rate by region (aged below 50)", x = 'Region', y = 'Divorce rate (%)')

#divorce rate by region for aged 50 and above
plot50 <- region1%>% filter(age >=50) %>% group_by(region) %>% summarise(divorce_rate = mean(binary_marstat)*100)%>% ggplot(aes(x = region, y = divorce_rate))+ geom_col() + theme(axis.text.x = element_text(angle = 30, hjust = 1)) + labs( title = "Divorce rate by region (aged 50 and above)", x = 'Region', y = 'Divorce rate (%)')
...
plotNA <- fd %>% select(dpcfamuk11, binary_marstat, tenure, scgpuk11c, binary_region_factor, children_class) %>% group_by(children_class)%>% summarise(divorce_rate = mean(binary_marstat)*100)%>%ggplot(aes(x = children_class, y = divorce_rate))+ geom_col(position = 'dodge') + theme(axis.text.x = element_text(angle = 60, hjust = 1)) + labs(x = 'Number of children in family')
